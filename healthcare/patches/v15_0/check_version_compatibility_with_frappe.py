...
def execute():
	...
	if frappe_v.startswith("14") and healthcare_v.startswith("15"):
		message = f"""
			The `develop` branch of Kaya EHR for LTC is no longer compatible with Frappe & ERPNext's `version-14`.
			Since you are using ERPNext/Frappe `version-14` please switch Kaya EHR for LTC app's branch to `version-14` and then proceed with the update.\n\t
