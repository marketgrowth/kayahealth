...
def setup_domain():
	...
	Add Long-term Care to active domains in Domain Settings
	"""
	domain = frappe.get_doc("Domain", "Long-term Care")
	domain.setup_domain()

	# update active domains
	if "Long-term Care" not in frappe.get_active_domains():
...
def setup_service_request_masters():
	records = [
		...
		},
		{
			...
			"definition": _(
				"The request represents a legally binding instruction authored by a client or RelatedPerson."
			),
			"official_url": "http://hl7.org/fhir/ValueSet/request-intent",
		},
		{
...
def get_observation_category_codes():
	return [
		...
		},
		{
			...
			"definition": _(
				"The results of observations generated by laboratories. Laboratory results are typically generated by laboratories providing analytic services in areas such as chemistry, hematology, serology, histology, cytology, anatomic pathology (including digital pathology), microbiology, and/or virology. These observations are based on analysis of specimens obtained from the client and submitted to the laboratory."
			),
			"official_url": "http://hl7.org/fhir/ValueSet/observation-category",
		},
		{
...
def get_medication_request_codes():
	return [
		...
		},
		{
			...
			"display": _("Entered in Error"),
			"definition": _(
				"The request was recorded against the wrong client or for some reason should not have been recorded (e.g. wrong medication, wrong dose, etc.). Some of the actions that are implied by the medication request may have occurred. For example, the medication may have been dispensed and the client may have taken some of the medication."
			),
			"official_url": "http://hl7.org/fhir/ValueSet/medicationrequest-status",
		},
		{
